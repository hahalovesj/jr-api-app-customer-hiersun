package com.hiersun.jewelry.api.service.impl;
...
import com.hiersun.jewelry.api.util.SerializationUtil;

import redis.clients.jedis.ShardedJedis;
import redis.clients.jedis.ShardedJedisPool;

@Service("redisBaseServiceImpl")
public class RedisBaseServiceImpl implements RedisBaseService {
	...
	public Serializable getObject(String key) {
		...
		try {
			...
			return SerializationUtils.deserialize(shardedJedis.get(keys));
		} catch (Exception ex) {
			logger.error("getList error.", ex);
			returnBrokenResource(shardedJedis);
		} finally {
	...
	public boolean setObject(String key, Serializable value) {
		...
		try {
			...
			shardedJedis = shardedJedisPool.getResource();
			byte[] keys = SerializationUtils.serialize(key);
			byte[] values = SerializationUtils.serialize(value);
			shardedJedis.set(keys, values);
			return true;
		} catch (Exception ex) {
			logger.error("setList error.", ex);
			returnBrokenResource(shardedJedis);
	...
	public boolean setObject(String key, int second, Serializable value) {
		...
		try {
			...
			byte[] keys = SerializationUtils.serialize(key);
			byte[] values = SerializationUtils.serialize(value);
			shardedJedis.setex(keys, second, values);
			return true;
		} catch (Exception ex) {
			logger.error("setList error.", ex);
			returnBrokenResource(shardedJedis);
	...
	public boolean setObj(String key, Object obj) {
		...
		try {
			shardedJedis = shardedJedisPool.getResource();
			shardedJedis.set(key.getBytes(), SerializationUtil.serialize(obj));
			return true;
		} catch (Exception ex) {
			logger.error("set error.", ex);
			returnBrokenResource(shardedJedis);
